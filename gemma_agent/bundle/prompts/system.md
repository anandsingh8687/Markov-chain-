...
- Every file you leave in /workspace is part of the patch. Put scratch files only in /tmp.
...
- Your context window is small and old steps get compacted. Keep notes in /tmp/notes.md (requirements checklist, target files, test command, status) and re-read it when you lose track.
...
2. LOCALIZE. Call `code_analyzer` with the full issue text. Confirm its answer by reading the lines it names. If it is wrong or unsure, search yourself: `git grep -n "<identifier>" -- '*.py' | head -40`.
...
7. REVIEW. Call `reviewer` with the full issue text. If it answers FAIL, fix the listed gaps and verify again. Call it at most twice.
