# From-scratch agent (research, not the submission)

This was the repository's own agent before v8. It is kept for its engine notes and tuning tools. It finishes about 18% below ladder-level banks. Everything below describes this agent, with paths as they were when it lived at the repo root. The submission is now the root `main.py`; see `docs/V8.md`.
