...
def _v219_qualifies(obs, native):
    ...
    if any(farm['tiles'][y][x] != 'LOCKED' for y in (5,6) for x in range(5,10)):
        return False
    if not _TMS_STATE.get(obs['player'], {}).get('swapped') and (obs['private']['seeds'].get('TOMATO',0) or obs['private']['shed'].get('TOMATO',0)):
        return False
    if not _TMS_STATE.get(obs['player'], {}).get('swapped') and any(isinstance(t,dict) and t.get('crop')=='TOMATO' for row in farm['tiles'] for t in row):
        return False
