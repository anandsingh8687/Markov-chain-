...
def _router(observation,step,state):
    ...
    if step>=144 and not state.get('day6'):
        ...
        _dflt=state['route']
        _cands=[c for c in (_R110_OLD_SHOPS.get(shops),_R108_SHOP_ROUTES.get(shops),_V92_TABLE.get(shops)) if c is not None and c in _ROUTES and c!=_dflt]
        if _RALT==1 and _cands: state['route']=_cands[0]
        if _RALT==2 and len(_cands)>1: state['route']=_cands[1]
        if _RALT==9: state['route']=_RALT_FIX
        state['day6']=True
