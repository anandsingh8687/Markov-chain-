...
# ------------------------------------------------------------------ lanes
CORNERS = {"NW": ((4, 4), -1, -1), "NE": ((5, 4), 1, -1), "SW": ((4, 5), -1, 1), "SE": ((5, 5), 1, 1)}
_LOCAL_LANES = (
    [(0, 0), (0, 1), (0, 2), (0, 3), (0, 4), (1, 4), (1, 3), (1, 2), (1, 1)],
    [(1, 0), (2, 0), (3, 0), (4, 0), (4, 1), (3, 1), (2, 1)],
    [(2, 2), (3, 2), (4, 2), (4, 3), (3, 3), (2, 3), (2, 4), (3, 4), (4, 4)],
)
LANES = {}
LANE_RANK = {}
for _q, ((_cx, _cy), _dx, _dy) in CORNERS.items():
    LANES[_q] = []
    for _li, _lane in enumerate(_LOCAL_LANES):
        pts = [(_cx + _dx * u, _cy + _dy * v) for u, v in _lane]
        LANES[_q].append(pts)
        for _k, _xy in enumerate(pts):
            LANE_RANK[_xy] = _k + (0.5 if _li == 2 else 0.0) * 2
...
class Foreman:
    ...
    def plan(self, obs, s, day):
        ...
        if P.get("LANES", 1):
            free.sort(key=lambda xy: (LANE_RANK.get(xy, 9), xy))
        else:
    ...
    def plan_routes(self, obs, s, day, hour, positions):
        if P.get("LANES", 1):
            return self.plan_lane_routes(obs, s, day, hour, positions)
        return self.plan_insert_routes(obs, s, day, hour, positions)

    def lane_bins(self, obs, s, day, hour, cap):
        me = int(obs["player"])
        quads = obs["farms"][me]["unlocked_quadrants"]
        items = []
        for q in quads:
            for lane in LANES.get(q, []):
                tiles = []
                w = 0
                for xy in lane:
                    c, o = self.tile_work(obs, s, xy, day, hour)
                    if c or o:
                        tiles.append(xy)
                        w += c + o
                if tiles:
                    corner = CORNERS[q][0]
                    travel = dist(corner, tiles[0]) + sum(dist(a, b) for a, b in zip(tiles, tiles[1:]))
                    items.append((w + travel + 2, q, tiles))
        items.sort(key=lambda it: -it[0])
        bins = []
        for w, q, tiles in items:
            for b in bins:
                extra = dist(b["end"], CORNERS[q][0]) + w
                if b["load"] + extra <= cap:
                    b["load"] += extra
                    b["tiles"] += tiles
                    b["end"] = tiles[-1]
                ...
                bins.append({"load": w, "tiles": list(tiles), "end": tiles[-1], "q": q})
        return bins

    def plan_lane_routes(self, obs, s, day, hour, positions):
        n = len(positions)
        cap = 24 - hour if hour > 0 else 23
        bins = self.lane_bins(obs, s, day, hour, cap)
        bins.sort(key=lambda b: -b["load"])
        routes = [[] for _ in range(n)]
        free = list(range(n))
        for b in bins:
            if not free:
                # more work than units: append to the lightest route
                i = min(range(n), key=lambda u: len(routes[u]))
                routes[i] += b["tiles"]
                continue
            i = min(free, key=lambda u: dist(positions[u], CORNERS[b["q"]][0]))
            free.remove(i)
            routes[i] = list(b["tiles"])
        s["routes"] = routes
        s["route_cost"] = [0] * n

    def plan_insert_routes(self, obs, s, day, hour, positions):
        ...
        for y in range(BOARD):
            for x in range(BOARD):
                ...
                if t is None:
                    ...
                    work += 1.0
        if P.get("LANES", 1):
            units = len(self.lane_bins(obs, s, day, 1, 23))
        else:
