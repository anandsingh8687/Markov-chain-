...
class Foreman:
    ...
    def plan(self, obs, s, day):
        ...
        if bp:
            ...
            if day <= P["ANIMAL_DAY_MAX"]:
                for a2 in ("COW", "SHEEP", "GOOSE"):
                    while herd[a2] < bp.get(a2, 0) and free:
                        xy = free.pop(0)
                        s["design"][xy] = a2
                        herd[a2] += 1
            n_ongoing = sum(1 for y in range(BOARD) for x in range(BOARD)
                            if isinstance(tiles[y][x], dict) and tiles[y][x].get("kind") == "PLANT"
                            and tiles[y][x]["crop"] == "STRAWBERRY")
            n_ongoing += sum(1 for c in s["planned"].values() if c == "STRAWBERRY")
            while free:
                if n_ongoing < bp.get("STRAWBERRY", 0) and day <= bp.get("STRAW_LAST_DAY", 13):
                    crop = "STRAWBERRY"
                    n_ongoing += 1
                else:
                    opts = [(self.crop_option(obs, s, c, day), c) for c in bp.get("FILL", ("WHEAT", "CARROT"))]
                    opts = [(o[0], c) for o, c in opts if o and o[1] > 0]
                    if not opts or day > 27:
                        break
                    crop = max(opts)[1]
                xy = free.pop(-1)
                s["planned"][xy] = crop
                s["supply"][crop] = s["supply"].get(crop, 0.0) + crop_plan(crop, 29 - day)[1]
            return
    ...
    def next_action(self, obs, s, xy, day, hour, inv, seeds):
        ...
        if kind == "PLANT":
            ...
            if day < 29 and water_needed(t, day) and (P["SMART_WATER"] or not t.get("watered_today")):
                return ["WATER"]
            if day < 29 and not P["SMART_WATER"] and not t.get("watered_today"):
                return ["WATER"]
    ...
    def tile_work(self, obs, s, xy, day, hour):
        ...
        if kind == "PLANT":
            ...
            if P["SMART_WATER"]:
                ...
                crit = 1 if wn == 2 else 0
                opt = 1 if wn == 1 else 0
            else:
    ...
    def plan_routes(self, obs, s, day, hour, positions):
        """Role split: ranchers take the animal tiles, crop workers the rest; cheapest insertion within each group."""
        me = int(obs["player"])
        tiles = obs["farms"][me]["tiles"]
        n = len(positions)
        ...
        for y in range(BOARD):
            for x in range(BOARD):
                ...
                if not (c or o):
                    continue
                t = tiles[y][x]
                is_animal = isinstance(t, dict) and ("animal" in t or t.get("kind") in ("COOP", "PASTURE"))
                (animal if is_animal else crop).append(((x, y), c, c + o))
        wa = sum(w + 1 for _, _, w in animal) + (3 if animal else 0)
        wc = sum(w + 1 for _, _, w in crop)
        n_a = 0
        if animal and n > 1:
            n_a = max(1, min(n - 1, int(round(n * wa / float(wa + wc)))))
        elif animal:
            n_a = 1
        order = sorted(range(n), key=lambda i: min(dist(positions[i], q) for q in ACCESS))
        ranchers = set(order[:n_a])
        routes = [[] for _ in range(n)]
        cost = [0.0] * n

        def insert(group, xy, w, strict):
            best = None
            for i in group:
                r = routes[i]
                pts = [positions[i]] + r
                for k in range(len(r) + 1):
                    a0 = pts[k]
                    b0 = r[k] if k < len(r) else None
                    add = dist(a0, xy) + (dist(xy, b0) - dist(a0, b0) if b0 is not None else 0) + w
                    if strict and cost[i] + add > horizon[i]:
                        continue
        ...
        for group, items in ((sorted(ranchers), animal), ([i for i in range(n) if i not in ranchers], crop)):
            if not group:
                group = list(range(n))
            items = sorted(items, key=lambda q: (-q[1], min(dist(q[0], a) for a in ACCESS)))
            for xy, c, w in items:
                if not insert(group, xy, w, True) and c:
                    insert(group, xy, w, False)
        s["routes"] = routes
        s["route_cost"] = cost
        s["ranchers"] = ranchers
    ...
    def dispatch(self, obs, s, day, hour, step):
        ...
        n = len(positions)
        if s.get("route_key") != (day, n) or hour in (1,) or hour in P["RESCUE_HOURS"]:
        ...
        for i, pos in enumerate(positions):
            ...
            if fetched:
                continue
            if i in s.get("ranchers", ()) and ((need_wheat > 0 and not inv.get("WHEAT", 0) and shed.get("WHEAT", 0) > 0) or
                                               any(not inv.get(a, 0) and shed.get(a, 0) > 0 for a in need_animal)):
    ...
    def market(self, obs, s, day, hour, step):
        ...
        if n_extra < P.get("MAX_EXTRA_LAND", 3) and room() and 1 <= hour <= 18:
