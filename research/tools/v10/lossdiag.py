...
def run(ep):
    ...
    ns = {'__name__': 'x'}; p = f'{SP}/pool/v9rt/main.py'; exec(compile(open(p).read(), p, 'exec'), ns)
    ...
    return {'ep': ep, 'opp': g['names'][op], 'm': r[0] - r[1], 'orig': g['r'][me] - g['r'][op], 'route': ns['_RT_LOG'].get('default'), 'shops': ns['_RT_LOG'].get('shops'), 'agg': dict(agg)}
