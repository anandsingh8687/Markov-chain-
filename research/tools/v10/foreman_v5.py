...
P = {
    ...
    "FERT_VALUE": 0.35,
    "LABOUR_FILL": 0.95,
    "MIN_TURN_VALUE": 4.0,               # expected rival supply relative to ours
    "CROP_HORIZON_PAD": 0,
}
...
class Foreman:
    ...
    def crop_option(self, obs, s, crop, day):
        ...
        value = units * price - CROPS[crop]["seed"]
        waters = tdays
        harvests = units if CROPS[crop]["ongoing"] else 1
        turns = (waters + harvests + 1) * (1.0 + P["WALK"])
