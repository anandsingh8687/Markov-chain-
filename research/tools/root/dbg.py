...
def _decide(obs, config=None):
    ...
    if not final_day and days_left > 1.5:
        ...
        if want_animal and len(orders) < maxord:
            ...
            import os as _o
            if _o.environ.get("KG_DEBUG") and hour == 0 and day % 3 == 0:
                print("d%02d %s own=%d cash=%.0f invest=%.0f runway=%.0f | "
                      "sustain=%d labor=%d feed=%d slots=%d | room=%d buy=%d"
                      % (day, a, own_kind.get(a, 0), money, invest, runway,
                         sustain.get(a, 0) - own_kind.get(a, 0) - qd.get(a, 0),
                         cap_labor - n_animals - qtot, cap_feed - n_animals - qtot,
                         len(structs) + len(empties) - qtot, room, wantn))
